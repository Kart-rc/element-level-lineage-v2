...
| Sidecar evidence path | **Sidecar container** in integration-test environments (feature flag via **AWS AppConfig** or the org's flag system, asserted on by the test-automation service, off in production by policy); emits `sidecar-test` envelopes → ingest gateway; `test.run.completed` events join evidence to the run's candidate artifact. Platform ships the sidecar image + conformance library | ADR-028 |
