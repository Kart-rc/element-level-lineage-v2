...
|---|---|---|---|
| 1 | **Coverage dashboard** (landing) | Per Business App: systems by coverage state (`automated` / `declared` / `uncovered` / `dormant` / `error` / `excluded:<class>` / `unclassified`), repo-classification distribution (ADR-029) with steward reclassify action, % edges by trust and band, review backlog, freshness. `not-observed` regions get first-class blind-spot treatment — never blank space | all (reclassify: steward+) |
